...
function Content() {
  ...
  const savePassword = async () => {
    if (form.site.length > 3 && form.username.length > 3 && form.password.length > 3) {
      ...
      if (editingId) {
        await fetch(`http://localhost:3000/passwords/${editingId}`, {
          method: 'PUT',
  ...
  const editPassword = (_id) => {
    const toEdit = passwordArray.find(i => i._id === _id);
    setform(toEdit);
    setEditingId(_id);
  ...
  return (
    ...
    <div>
      ...
      <div className="flex flex-col items-center mt-3 sm:mt-6 w-[85%] sm:w-[65%] mx-auto gap-4">
        <input
          type="text"
          name="site"
      ...
      <div className="container mt-5 mx-auto w-[85%] sm:w-[65%] min-h-[36vh] sm:min-h-[40vh] bg-white p-6 rounded-2xl shadow-md mb-[10px]">
                    ...
                    <tr key={index} className="hover:bg-yellow-50 transition">
                      ...
                      <td className="py-3 px-2 sm:px-5">
                        <div className="flex gap-1 sm:gap-3 items-center h-full">
                          <button className="bg-yellow-400 text-black p-1 sm:px-3 sm:py-1 rounded-full text-xs font-semibold hover:bg-yellow-300 transition hover:cursor-pointer" onClick={() => { editPassword(item._id) }} >
                            <FaEdit className="text-xs" />
                          </button>
                          <button className="bg-red-400 text-white p-1 sm:px-3 sm:py-1 rounded-full text-xs font-semibold hover:bg-red-300 transition hover:cursor-pointer" onClick={() => { deletePassword(item._id) }}>
